...
interface Props {
  ...
  onSendToFarmer?: (instruments: string[]) => void;
}

export const ScreenerTab: React.FC<Props> = ({ token, results, setResults, onSendToFarmer }) => {
  ...
  const actionBody = (row: ScreenerResult) => (
    <div className="flex gap-1">
      <Button
        ...
        onClick={() => {
          const api = (window as any).electronAPI;
          api?.runBacktest?.(row.figi || row.uid, '', '', '', token, {
  ...
  const sendAllToFarmer = () => {
    const uids = successResults.map(r => r.uid);
    if (uids.length > 0 && onSendToFarmer) {
      onSendToFarmer(uids);
    }
  };
  ...
  return (
    <Card className="surface-ground p-2">
      ...
      {/* Подробное описание работы скринера */}
      ...
      <div className="flex align-items-center flex-wrap gap-3 mb-3">
        <div className="flex align-items-center gap-2">
          ...
          <InputNumber value={filters.minVolume} onValueChange={(e) => setFilters({ ...filters, minVolume: e.value ?? 10000 })} min={0} step={1000} className="p-inputtext-sm" style={{ width: '140px' }} />
        </div>
        <div className="flex align-items-center gap-2">
          <label className="mb-0">Макс. ширина VA (% от POC)</label>
          <InputNumber value={filters.maxVA} onValueChange={(e) => setFilters({ ...filters, maxVA: e.value ?? 2.0 })} min={0.1} step={0.1} mode="decimal" className="p-inputtext-sm" style={{ width: '110px' }} />
        </div>
        <div className="flex align-items-center gap-2">
          <label className="mb-0">Мин. сила POC (доля объёма VA)</label>
          <InputNumber value={filters.minPOC} onValueChange={(e) => setFilters({ ...filters, minPOC: e.value ?? 0.15 })} min={0} max={1} step={0.01} mode="decimal" className="p-inputtext-sm" style={{ width: '110px' }} />
        </div>
        <Button label={loading ? 'Анализ...' : 'Запустить скрининг'} icon={loading ? 'pi pi-spin pi-spinner' : 'pi pi-search'} onClick={handleRun} disabled={loading} className="p-button-sm p-1 px-3" />
      </div>
      ...
      {successResults.length > 0 && (
        <div className="mb-3">
          <div className="flex align-items-center justify-content-between mb-2">
            ...
            <Button label="Отправить в фермер" icon="pi pi-send" className="p-button-sm p-button-warning" onClick={sendAllToFarmer} />
          </div>
          <DataTable value={successResults} className="p-datatable-sm" stripedRows responsiveLayout="scroll" style={{ fontSize: '0.85rem' }}>
            ...
            <Column field="poc" header="POC" body={(row) => row.poc != null ? row.poc.toFixed(2) : '—'} />
            <Column field="vaWidthPercent" header="VA ширина %" body={(row) => row.vaWidthPercent != null ? row.vaWidthPercent + '%' : '—'} />
            <Column field="pocStrength" header="Сила POC" body={(row) => row.pocStrength != null ? row.pocStrength : '—'} />
            <Column field="avgVolumePerCandle" header="Объём (ср.)" body={(row) => row.avgVolumePerCandle != null ? row.avgVolumePerCandle.toLocaleString() : '—'} />
            <Column field="vah" header="VAH" body={(row) => row.vah != null ? row.vah.toFixed(2) : '—'} />
            <Column field="val" header="VAL" body={(row) => row.val != null ? row.val.toFixed(2) : '—'} />
            <Column body={actionBody} header="Действия" style={{ minWidth: '80px' }} />
          </DataTable>
