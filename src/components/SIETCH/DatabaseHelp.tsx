...
access_level: public`}</CodeBlock>
        ...
        <TabPanel header="Чувствительные данные">
          <Panel header="Зачем отдельная база" className="mb-3">
            <p>
              <b>Паспорта, СНИЛС, даты рождения, домашние адреса, личные телефоны и почты,
              банковские счета</b> — это чувствительные данные. Их нельзя хранить в основной
              БД по нескольким причинам:
            </p>
            <ul className="pl-4">
              <li>
                они попадают в общий дамп и уезжают вместе с публичными данными;
              </li>
              <li>
                основная БД не шифруется — если файл утечёт, sensitive-записи будут
                доступны в открытом виде;
              </li>
              <li>
                юридические требования — данные требуют явного основания хранения и
                срока, а также защиты.
              </li>
            </ul>
            <p className="mt-2">
              Поэтому sensitive-записи хранятся в <b>отдельном зашифрованном файле</b>{' '}
              <code>sensitive_data.db</code> со своими правилами доступа.
            </p>
          </Panel>

          <Panel header="Что хранить, а что нет" className="mb-3">
            <p><b>Можно:</b></p>
            <ul className="pl-4">
              <li>паспортные данные (серия, номер, кем выдан);</li>
              <li>СНИЛС, ИНН, водительское удостоверение;</li>
              <li>дата рождения;</li>
              <li>домашний адрес, адрес регистрации;</li>
              <li>личный телефон и личная почта;</li>
              <li>банковские счета.</li>
            </ul>
            <p className="mt-2"><b>Не хранить в основной БД:</b></p>
            <ul className="pl-4">
              <li>всё перечисленное выше — только в sensitive-раздел.</li>
            </ul>
            <p className="mt-2 text-500">
              Если по ошибке добавили sensitive-атрибут в обычное наблюдение — удалите
              его и заведите заново в разделе «Чувствительные данные».
            </p>
          </Panel>

          <Panel header="Как открыть" className="mb-3">
            <ol className="pl-4">
              <li>Откройте окно БД → кликните по строке любой <b>сущности</b>.</li>
              <li>
                В футере диалога нажмите иконку <b>щита</b> (крайняя слева) —
                откроется раздел «Чувствительные данные».
              </li>
              <li>
                Sensitive доступны только для сущностей. Для связей, наблюдений и
                источников кнопки нет.
              </li>
            </ol>
          </Panel>

          <Panel header="Первый запуск — мастер настройки" className="mb-3">
            <p>При первом открытии появится мастер. Нужно выбрать режим защиты:</p>

            <p className="mt-2">
              <b>Режим 1. Фраза восстановления (рекомендуется).</b>
            </p>
            <ul className="pl-4">
              <li>10–24 слова через пробел.</li>
              <li>
                Можно <b>сгенерировать</b> случайную фразу (кнопка «Сгенерировать») —
                слова берутся из словаря BIP39 (English) или русского словаря.
              </li>
              <li>
                Можно ввести свою фразу — любые слова, но не менее 10.
              </li>
              <li>
                Ключ шифрования — производный от фразы (PBKDF2-SHA256, 600 000 итераций),
                каждое значение шифруется отдельно (AES-256-GCM).
              </li>
              <li>
                <b>Портативность:</b> файл <code>sensitive_data.db</code> можно перенести
                на другую машину и расшифровать той же фразой.
              </li>
            </ul>

            <p className="mt-3">
              <b>Режим 2. Системный ключ.</b>
            </p>
            <ul className="pl-4">
              <li>Фраза не нужна.</li>
              <li>
                Случайный 32-байтный ключ сохраняется через системный keyring (Windows
                DPAPI / macOS Keychain / Linux libsecret).
              </li>
              <li>
                <b>Не портативен:</b> файл нельзя расшифровать на другой машине.
                Используйте, если sensitive-данные не покидают эту машину.
              </li>
            </ul>

            <p className="mt-3"><b>Чекбокс авторазблокировки.</b></p>
            <ul className="pl-4">
              <li>
                Если включён — фраза (или системный ключ) сохраняется в keyring и при
                следующем запуске sensitive открывается без ввода.
              </li>
              <li>
                Если выключен — фразу придётся вводить при каждом запуске приложения.
              </li>
              <li>
                На Linux без keyring авторазблокировка недоступна — фразу вводим раз в
                сессию.
              </li>
            </ul>
          </Panel>

          <Panel header="Куда сохранить фразу" className="mb-3">
            <p>
              Фраза восстановления — <b>единственный</b> способ получить доступ к
              sensitive-данным. Храните её в надёжном месте:
            </p>
            <ul className="pl-4">
              <li>менеджер паролей (KeePassXC, Bitwarden и т.п.);</li>
              <li>распечатанная бумажная копия в сейфе;</li>
              <li>зашифрованный файл на офлайн-носителе.</li>
            </ul>
            <p className="mt-2"><b>Не храните фразу:</b></p>
            <ul className="pl-4">
              <li>в <code>.env</code> или в исходниках — попадёт в git;</li>
              <li>в заметках на рабочем столе;</li>
              <li>в облаке без шифрования;</li>
              <li>
                рядом с самим файлом <code>sensitive_data.db</code> — тогда шифрование
                теряет смысл.
              </li>
            </ul>
          </Panel>

          <Panel header="Передача данных через торрент" className="mb-3">
            <p>
              Sensitive-данные можно передавать вместе с публичными через торрент:
            </p>
            <ol className="pl-4">
              <li>
                Файл <code>sensitive_data.db</code> упаковывается вместе с остальными
                данными и раздаётся через BitTorrent.
              </li>
              <li>
                Фраза восстановления <b>передаётся отдельным каналом</b>: Signal,
                GPG-письмо, личная встреча. Никогда — в том же торренте.
              </li>
              <li>
                Получатель импортирует <code>sensitive_data.db</code>, вводит фразу,
                включает авторазблокировку у себя.
              </li>
            </ol>
            <p className="mt-2">
              Даже если торрент-файлы утекут к третьим лицам — sensitive-записи
              останутся зашифрованными, потому что без фразы их не расшифровать.
            </p>
          </Panel>

          <Panel header="Смена машины" className="mb-3">
            <p>При переносе sensitive на другую машину:</p>
            <ul className="pl-4">
              <li>
                <b>Переносится:</b> <code>sensitive_data.db</code>.
              </li>
              <li>
                <b>Не переносится:</b> <code>sensitive_passphrase.dat</code> — этот файл
                привязан к keyring старой машины и там останется.
              </li>
              <li>
                На новой машине при первом открытии sensitive вводится фраза
                восстановления (или, если был выбран системный ключ — данные восстановить
                нельзя, потому что ключ привязан к старой машине).
              </li>
              <li>
                После успешного ввода можно снова включить авторазблокировку — уже на
                новой машине.
              </li>
            </ul>
          </Panel>

          <Panel header="Забыли фразу" className="mb-3">
            <p>
              <b>Данные восстановить невозможно.</b> Никакого «восстановления пароля»
              не существует — только сам пользователь знает фразу, и никто больше.
            </p>
            <p className="mt-2">Что можно сделать:</p>
            <ul className="pl-4">
              <li>
                Если фраза была сохранена в keyring и доступ к keyring есть — нажать
                «Разблокировать» в диалоге, при необходимости снять галочку «Спрашивать
                каждый раз».
              </li>
              <li>
                Если keyring потерян — можно только стереть <code>sensitive_data.db</code>{' '}
                и начать заново. Все sensitive-записи будут потеряны.
              </li>
            </ul>
          </Panel>

          <Panel header="Юридический дисклеймер" className="mb-3">
            <div
            ...
            >
              <p>
                <b>Не храните sensitive-данные без законного основания.</b> Каждая
                запись требует:
              </p>
              <ul className="pl-4">
                <li>
                  <b>Основание хранения</b> (<code>legal_basis</code>) — служебная
                  необходимость, согласие субъекта, требование закона и т.п.
                </li>
                <li>
                  <b>Срок хранения</b> (<code>retention_until</code>) — дата, до которой
                  данные должны быть удалены.
                </li>
              </ul>
              <p className="mt-2">
                При отсутствии обоснования хранение может нарушать 152-ФЗ (РФ) и аналогичные
                законы других юрисдикций (GDPR и др.). Приложение технически не может
                проверить законность — это ответственность пользователя.
              </p>
            </div>
          </Panel>

          <Panel header="Как добавить запись" className="mb-3">
            <ol className="pl-4">
              <li>Открыть диалог sensitive для нужной сущности.</li>
              <li>Если хранилище заблокировано — ввести фразу.</li>
              <li>Нажать кнопку <b>«Добавить»</b> сверху.</li>
              <li>
                Заполнить поля:
                <ul className="pl-4 mt-1">
                  <li><b>Поле</b> — выпадающий список (passport, snils, dob, ...) с возможностью ввести своё;</li>
                  <li><b>Значение</b> — само значение (скрыто точками в списке);</li>
                  <li><b>Основание хранения</b> — обязательно;</li>
                  <li><b>Хранить до</b> — дата (опционально);</li>
                  <li><b>Заметки</b> — опционально.</li>
                </ul>
              </li>
              <li>Нажать <b>«Сохранить»</b>.</li>
              <li>
                Значение по умолчанию скрыто (<code>••••••••</code>). Нажмите иконку
                глаза, чтобы показать. При закрытии диалога все раскрытые значения
                сбрасываются.
              </li>
            </ol>
          </Panel>

          <Panel header="Блокировка и разблокировка">
            <ul className="pl-4">
              <li>
                Кнопка <b>«Заблокировать»</b> в футере — временно закрывает доступ, не
                удаляя фразу из keyring. В следующий раз достаточно нажать
                «Разблокировать».
              </li>
              <li>
                Кнопка <b>«Забыть сохранённую фразу»</b> (в режиме разблокировки) —
                удаляет фразу из keyring. При следующем запуске потребуется ввод.
              </li>
              <li>
                В режиме «Системный ключ» разблокировка происходит автоматически —
                фразы нет, кнопка «Забыть» не отображается.
              </li>
            </ul>
          </Panel>
        </TabPanel>
