...
export const EntityDetailsContent: React.FC<EntityDetailsContentProps> = ({
  ...
  const isCourtCase = data.entity.type === 'court_case';
  const repRelations = isCourtCase
    ? (data.relations_via || []).filter((r: any) => r.predicate === 'represents')
    : [];
  ...
  return (
    <>
      ...
      {repRelations.length > 0 && (
        <div className="mt-3">
          <div className="flex align-items-center gap-2 mb-2">
            <i className="pi pi-users" />
            <span className="font-bold">Представители ({repRelations.length})</span>
          </div>
          <RepresentativesTable
            relations={repRelations}
            onOpenEntity={(id) => openDialog('entity', id)}
            onOpenRelation={(id) => openDialog('relation', id)}
          />
        </div>
      )}
