...
import { CodeSourceType, CodeSnippet, CodeFileReference, PromptTemplate } from '@/types/promptgenerator';
...
import { Divider } from 'primereact/divider';
import { CodeSourceType, CodeSnippet, CodeFileReference } from '@/types/promptgenerator';
import { PackageJson } from '@/types/types';
...
export const CodeContextSection: React.FC<CodeContextSectionProps> = ({ template, onUpdate, packageJson, configFiles }) => {
  const enabledOptions = [
    { label: 'Вкл.', value: true },
    { label: 'Выкл.', value: false }
  ];
  ...
  // Тип для результата чтения файлов (полный, как в main)
  interface ReadFileResult {
    path: string;
    filename: string;
    content: string;
    ...
    if (!codeContext.codeSources) {
      switch (currentSourceType) {
        ...
        case CodeSourceType.FilePaths:
          return [{ path: '', purpose: '', process: 'main' }];
        default:
          return '';
      }
    }
  ...
  const handleAddFromFiles = async () => {
    ...
    setFileLoading(true);
    try {
      // 1. Получаем пути к выбранным файлам
      const filePaths: string[] | null = await electronAPI.openFilePicker();
      if (!filePaths || filePaths.length === 0) {
        console.log('Файлы не выбраны');
        return;
      ...
      // В зависимости от типа источника — разное поведение
      if (currentSourceType === CodeSourceType.FilePaths) {
        ...
        const newFiles: CodeFileReference[] = filePaths.map((filePath: string) => {
          ...
          return {
            path: filePath,
            purpose: fileName,       // временное назначение — имя файла
            process: 'main',
          };
        });
        const current = Array.isArray(codeContext.codeSources)
        ...
        if (currentSourceType === CodeSourceType.Snippet) {
          // Один фрагмент — добавляем содержимое всех выбранных файлов в текстовое поле
          let newCode = typeof codeContext.codeSources === 'string' ? codeContext.codeSources : '';
          files.forEach((file) => {
            if (newCode) newCode += '\n\n';
            newCode += `// ${file.filename}\n${file.content}`;
          ...
          // Если язык ещё не задан, берём из первого файла
          if (!codeContext.language && files.length > 0 && files[0].language) {
            handleCodeContextChange('language', files[0].language);
          }
        } else if (currentSourceType === CodeSourceType.MultipleSnippets) {
          // Несколько фрагментов — создаём по объекту CodeSnippet на каждый файл
          const newSnippets: CodeSnippet[] = files.map((file) => ({
            code: file.content,
            purpose: file.filename,
            process: 'main',
            dependencies: [],
            problemDescription: '',
            // language: file.language,   // раскомментируйте, если CodeSnippet допускает поле language
          }));
          const currentSnippets = Array.isArray(codeContext.codeSources)
            ? [...(codeContext.codeSources as CodeSnippet[])]
            : [];
          ...
          // Так же автоматически проставляем общий язык секции, если не указан
          if (!codeContext.language && files.length > 0 && files[0].language) {
            handleCodeContextChange('language', files[0].language);
          }
        }
  ...
  const handleLoadPackageJson = async () => {
    try {
      if (!packageJson) {
        console.error('package.json не загружен');
        return;
      }
      
      console.log('Получен package.json:', packageJson);
      
      // 1. Заполняем зависимости (объединяем все типы или только нужные)
      const allDeps = {
        ...(packageJson.dependencies || {}),
        ...(packageJson.devDependencies || {}),   // обычно для промпта нужны только production‑зависимости
        ...(packageJson.peerDependencies || {}),
      };
      const depList = Object.entries(allDeps).map(
        ([name, version]) => `${name}@${version}`
      );

      // Обновляем поле с зависимостями
      handleCodeContextChange('dependencies', depList);

      // 2. Определяем основной язык программирования по ключевым пакетам
      if (!codeContext.language) {
        if (packageJson.dependencies?.typescript || packageJson.devDependencies?.typescript) {
          handleCodeContextChange('language', 'TypeScript');
        } else if (packageJson.dependencies?.react) {
          // React обычно с TypeScript или JS; точнее можно проверить наличие tsconfig
          handleCodeContextChange('language', 'JavaScript/TypeScript');
        }
        // ... другие эвристики
      }

      // 3. Можно также заполнить purpose или problemDescription при необходимости
      // ...
    } catch (error) {
      console.error('Ошибка загрузки package.json:', error);
    }
  };

  // Загрузка package.json как фрагмент кода
  // !!! НЕОБХОДИМО ДОБАВИТЬ КНОПКУ ДЛЯ ЗАГРУЗКИ
  const handleLoadPackageJsonAsSnippet = async () => {
    try {
      if (!packageJson) {
        console.error('package.json не загружен');
        return;
      ...
      handleCodeContextChange('codeSources', formattedJson);
      handleCodeContextChange('language', 'json');
    } catch (error) {
      console.error(error);
    }
    ...
    <section className="p-mb-6">
      ...
      {isSectionEnabled && (
        <div>
          ...
          {
            // Поле «Процесс» показываем только для Snippet
          }
          {(currentSourceType === CodeSourceType.Snippet) && (
            <div className="mb-4 text-sm">
              <label className="block font-medium text-700 mb-2">Процесс:</label>
          ...
          {
            // Поле «Фрагмент кода» показываем только для Snippet}
          {currentSourceType === CodeSourceType.Snippet && (
            <div className="mb-4 text-sm">
              <label className="block font-medium text-700 mb-2">Фрагмент кода:</label>
              <Textarea
              ...
              <Button
                ...
                loading={fileLoading}
                onClick={handleAddFromFiles}
                className="p-button-accent p-button-sm mt-2"
              />
            </div>
          )}
          ...
          {currentSourceType === CodeSourceType.MultipleSnippets && (
            <div className="mb-4 text-sm">
              <label className="block font-medium text-700 mb-2">Фрагменты кода:</label>
              {Array.isArray(safeCodeSources) && (safeCodeSources as CodeSnippet[]).map((snippet, index) => (
                <div key={index} className="mb-3 p-3 border-round border-hint border-1" style={{background: 'var(--tg-theme-section-bg-color)'}}>
                  <div className="flex justify-content-end">
                    <Button
                      icon="pi pi-trash"
                      className="p-button-accent p-button-sm"
                      onClick={() => {
                        const updatedSnippets = (safeCodeSources as CodeSnippet[]).filter((_, i) => i !== index);
                        handleCodeContextChange('codeSources', updatedSnippets);
                      }}
                      aria-label="Удалить фрагмент"
                    />
                  </div>
                  <div className="mb-2">
                    ...
                    <Textarea
                      value={snippet.code || ''}
                      onChange={(e) => {
                        const updatedSnippets = [...safeCodeSources as CodeSnippet[]];
                        updatedSnippets[index].code = e.target.value;
                        handleCodeContextChange('codeSources', updatedSnippets);
                      }}
                      rows={4}
                      placeholder="Вставьте код..."
                  ...
                  <div className="mb-2">
                    ...
                    <InputText
                      ...
                      onChange={(e) => {
                        const updatedSnippets = [...safeCodeSources as CodeSnippet[]];
                        updatedSnippets[index].purpose = e.target.value;
                        handleCodeContextChange('codeSources', updatedSnippets);
                      }}
                      placeholder="Назначение фрагмента..."
                      className="w-full text-sm"
                  ...
                  <div className="mb-2">
                    ...
                    <Dropdown
                      ...
                      onChange={(e) => {
                        const updatedSnippets = [...safeCodeSources as CodeSnippet[]];
                        updatedSnippets[index].process = e.value;
                        handleCodeContextChange('codeSources', updatedSnippets);
                      }}
                      placeholder="Выберите процесс"
                      className="w-full text-sm"
                  ...
                  <div className="mb-2">
                    ...
                    <InputText
                      ...
                      onChange={(e) => {
                        const updatedSnippets = [...safeCodeSources as CodeSnippet[]];
                        updatedSnippets[index].dependencies = e.target.value
                          .split(',')
                          .map(dep => dep.trim())
                          .filter(dep => dep);
                        handleCodeContextChange('codeSources', updatedSnippets);
                  ...
                  <div className="mb-2">
                    ...
                    <Textarea
                      ...
                      onChange={(e) => {
                        const updatedSnippets = [...safeCodeSources as CodeSnippet[]];
                        updatedSnippets[index].problemDescription = e.target.value;
                        handleCodeContextChange('codeSources', updatedSnippets);
                      }}
                      rows={2}
                      placeholder="Описание проблемы в этом фрагменте..."
              ...
              <Button
                ...
                loading={fileLoading}
                onClick={handleAddFromFiles}
                className="p-button-accent p-button-sm mt-2 ml-2"
              />
            </div>
          )}
          ...
          {currentSourceType === CodeSourceType.FilePaths && (
            <div className="mb-4 text-sm">
              <label className="block font-medium text-700 mb-2">Пути к файлам:</label>
              {Array.isArray(safeCodeSources) && (safeCodeSources as CodeFileReference[]).map((file, index) => (
                <div key={index} className="mb-3 p-3 border-round border-hint border-1" style={{background: 'var(--tg-theme-section-bg-color)'}}>
                  <div className="flex justify-content-end">
                    <Button
                      icon="pi pi-trash"
                      className="p-button-accent p-button-sm"
                      onClick={() => {
                        const updatedFiles = (safeCodeSources as CodeFileReference[]).filter((_, i) => i !== index);
                        handleCodeContextChange('codeSources', updatedFiles);
                      }}
                      aria-label="Удалить файл"
                    />
                  </div>
                  <div className="mb-2">
                    ...
                    <InputText
                      value={file.path || ''}
                      onChange={(e) => {
                        const updatedFiles = [...safeCodeSources as CodeFileReference[]];
                        updatedFiles[index].path = e.target.value;
                        handleCodeContextChange('codeSources', updatedFiles);
                      }}
                      placeholder="src/main.ts"
                      className="w-full text-sm"
                  ...
                  <div className="mb-2">
                    ...
                    <InputText
                      ...
                      onChange={(e) => {
                        const updatedFiles = [...safeCodeSources as CodeFileReference[]];
                        updatedFiles[index].purpose = e.target.value;
                        handleCodeContextChange('codeSources', updatedFiles);
                      }}
                      placeholder="Основная точка входа"
                      className="w-full text-sm"
                  ...
                  <div className="mb-2">
                    ...
                    <Dropdown
                      ...
                      onChange={(e) => {
                        const updatedFiles = [...safeCodeSources as CodeFileReference[]];
                        updatedFiles[index].process = e.value;
                        handleCodeContextChange('codeSources', updatedFiles);
                      }}
                      placeholder="Выберите процесс"
                      className="w-full text-sm"
                  ...
                  <div className="mb-2">
                    ...
                    <Textarea
                      ...
                      onChange={(e) => {
                        const updatedFiles = [...safeCodeSources as CodeFileReference[]];
                        updatedFiles[index].problemDescription = e.target.value;
                        handleCodeContextChange('codeSources', updatedFiles);
                      }}
                      rows={2}
                      placeholder="Проблемы в этом файле..."
          ...
          {
            // Поле «Назначение» показываем только для Snippet
          }
          {(currentSourceType === CodeSourceType.Snippet) && (
            <div className="mb-4 text-sm">
              <label className="block font-medium text-700 mb-2">Назначение:</label>
          ...
          {
            // Поле «Зависимости» показываем только для Snippet
          }
          {(currentSourceType === CodeSourceType.Snippet) && (
            <div className="mb-4 text-sm">
              <label className="block font-medium text-700 mb-2">Зависимости:</label>
              <InputText
                ...
                onChange={(e) =>
                  handleCodeContextChange(
                    'dependencies',
                    e.target.value.split(',').map(dep => dep.trim()).filter(dep => dep)
                  )
                }
                placeholder="React, Express, Axios..."
                className="w-full text-sm"
          ...
          {
            // Поле «Описание проблемы» показываем только для Snippet
          }
          {(currentSourceType === CodeSourceType.Snippet) && (
            <div className="mb-4 text-sm">
            <label className="block font-medium text-700 mb-2">Описание проблемы:</label>
