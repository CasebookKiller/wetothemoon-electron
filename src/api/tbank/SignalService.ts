...
export class SignalServiceClient {
  ...
  constructor(
    ...
    this.storagePrefix = options?.storagePrefix ?? 'tinkoff_signal_cache';
  ...
  async getSignals(
    ...
    return this.requestWithCache<GetSignalsResponse>('GetSignals', request, options?.token, options?.signal);
  }
  ...
  async getStrategies(
    ...
    return this.requestWithCache<GetStrategiesResponse>('GetStrategies', request, options?.token, options?.signal);
  }
  ...
  private async requestWithRetry<T>(method: string, body: unknown, token?: string, signal?: AbortSignal, attempt = 0): Promise<T> {
    const url = `${this.baseUrl}/rest/tinkoff.public.invest.api.contract.v1.SignalService/${method}`;
  ...
  private log(msg: string): void { this.logger?.(`[SignalService] ${msg}`); }
}