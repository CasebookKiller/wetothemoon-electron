...
export async function collectPersonConnectionsDetails(page: Page, slug: string): Promise<any> {
  ...
  try {
    response = await page.goto(connectionsUrl, { waitUntil: 'domcontentloaded', timeout: 60000 });
  } catch (e) {
  ...
  // === Извлечение данных (та же логика, что и для ЮЛ/ИП) ===
    ...
    similarItems.forEach((similarItem) => {
      ...
      subItems.forEach((subItem) => {
        ...
        if (title || organizations.length > 0) {
          connections.push({ title, description, organizations });
        }
      });
    });

    return { total_organizations: totalText, connections };
  });
