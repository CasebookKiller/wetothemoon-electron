...
function getOrCreateClient(streamKey) {
...
var registerMarketdataStreamHandlers = () => {
	...
	electron.ipcMain.handle("md-stream-start", async (_, streamKey, token, requestBody) => {
		...
		const scheduleReconnect = () => {
			...
			state.retryCount++;
			console.log(`[Main] Переподключение ${streamKey} через ${delay / 1e3}с (попытка ${state.retryCount})`);
			state.retryTimeout = setTimeout(connect, delay);
		};
		connect();
	});
	electron.ipcMain.handle("md-stream-stop", async (_, streamKey) => {
...
var registerTradingAssistantHandlers = (historicalLoader, profileEngine, getToken, strategyManager, compositeProfileService, orderFlowEngine) => {
	...
	electron.ipcMain.handle("trading-assistant:send-manual-order", async (_, params) => {
		if (!orderManagerInstance) return {
			success: false,
			error: "OrderManager не инициализирован"
		};
...
async function applySouFilters(page, filters) {
	...
	if (filters.categories && Array.isArray(filters.categories) && filters.categories.length > 0) for (const cat of filters.categories) {
	...
	if (filters.regions && Array.isArray(filters.regions) && filters.regions.length > 0) for (const region of filters.regions) {
	...
	if (filters.results && Array.isArray(filters.results) && filters.results.length > 0) for (const result of filters.results) {
	...
	if (filters.matchLevel && Array.isArray(filters.matchLevel) && filters.matchLevel.length > 0) for (const level of filters.matchLevel) {
...
async function collectTrademarksDetails(page, companyId, options = {}) {
...
async function collectLeasingDetails(page, companyId, options = {}) {
	...
	try {
		const m = (await page.locator("div.export-data__text").first().innerText()).match(/Найдено\s*([\d\s]+)\s*договоров? лизинга/);
	...
	while (currentPage <= maxPages && collected < maxTotalCases) {
		...
		for (let i = 0; i < itemCount && collected < maxTotalCases; i++) {
			...
			for (let j = 0; j < triggerCount; j++) {
				...
				try {
					const parentTrigger = link.locator("..");
			...
			const basic = await item.evaluate((li) => {
				...
				const title = getText(".snippet__row-value.--title");
				const fields = {};
				li.querySelectorAll("div.snippet__row").forEach((row) => {
					...
					if (key && !key.includes("--subtitle")) fields[key] = value;
				});
				const leaseItems = [];
				li.querySelectorAll("div.snippet__block").forEach((block) => {
					const innerRows = block.querySelectorAll(".snippet__row");
					if (innerRows.length > 0) {
						if (Array.from(innerRows).some((row) => {
				...
				const changes = [];
				li.querySelectorAll(".leasing-changes-item").forEach((changeItem) => {
					...
					if (container) {
						...
						container.querySelectorAll(".snippet__block-related").forEach((block) => {
							const related = [];
...
async function collectPledgesDetails(page, companyId, options = {}) {
	...
	try {
		const m = (await page.locator("div.export-data__text").first().innerText()).match(/Найдено\s*([\d\s]+)\s*сообщений о залогах/);
	...
	while (currentPage <= maxPages && collected < maxTotalCases) {
		...
		for (let i = 0; i < itemCount && collected < maxTotalCases; i++) {
			...
			for (let j = 0; j < buttonCount; j++) try {
			...
			for (let j = 0; j < triggerCount; j++) try {
			...
			const parsed = await item.evaluate((li) => {
				...
				const pledgedItems = [];
				li.querySelectorAll(".snippet__block-mortaged").forEach((block) => {
					const item = {};
					block.querySelectorAll(".snippet__row").forEach((row) => {
						const key = row.querySelector(".snippet__row-key")?.textContent?.trim() || "";
						const value = row.querySelector(".snippet__row-value")?.textContent?.trim() || "";
				...
				const documents = [];
				li.querySelectorAll("a.snippet__link--document").forEach((link) => {
					const text = link.textContent?.trim() || "";
					if (text) documents.push(text);
				});
				const changes = [];
				li.querySelectorAll(".pledge-changes-item").forEach((changeItem) => {
					...
					if (container) container.querySelectorAll(".snippet__row").forEach((row) => {
						const key = row.querySelector(".snippet__row-key")?.textContent?.trim() || "";
						const value = row.querySelector(".snippet__row-value")?.textContent?.trim() || "";
						if (key) details.push({
							key,
...
async function collectFactsDetails(page, companyId, options = {}) {
	...
	try {
		const m = (await page.locator("div.export-data__text, .filters-results__head").first().innerText()).match(/Найдено\s*([\d\s]+)\s*сообщений?/);
		if (m) data.total_messages = m[1].replace(/\s/g, "");
	} catch (e) {
	...
	while (currentPage <= maxPages && collected < maxTotalCases) {
		...
		for (let i = 0; i < itemCount && collected < maxTotalCases; i++) {
			...
			for (let j = 0; j < forceCount; j++) try {
			...
			const parsed = await item.evaluate((li) => {
				...
				const publisherHref = publisherLink ? publisherLink.href || "" : "";
				const fields = {};
				li.querySelectorAll("div.snippet__row").forEach((row) => {
					const keyEl = row.querySelector(".snippet__row-key");
					const valueEl = row.querySelector(".snippet__row-value");
					...
					const key = keyEl.textContent?.trim() || "";
					const value = valueEl.textContent?.trim() || "";
					if (key && !key.includes("--subtitle") && key !== "Комментарий публикатора") fields[key] = value;
				});
				let comment = "";
				...
				if (creditorsContainer) {
					creditorsContainer.querySelectorAll("a.snipper__link").forEach((link) => {
						creditors.push({
					...
					creditorsContainer.querySelectorAll("span").forEach((span) => {
						...
						if (text && !span.querySelector("a")) creditors.push({
							name: text,
							href: ""
						});
					});
				...
				const documents = [];
				li.querySelectorAll(".docs-item").forEach((doc) => {
					const text = doc.textContent?.trim() || "";
					const guid = doc.getAttribute("data-file-guid") || "";
					if (text || guid) documents.push({
						name: text,
						guid
					});
				});
	...
	while (currentPage <= maxPages && collected < maxTotalCases) {
		...
		for (let i = 0; i < itemCount && collected < maxTotalCases; i++) {
			...
			const parsed = await item.evaluate((li) => {
				...
				rows.forEach((row) => {
					if ((row.querySelector(".snippet__row-key")?.textContent?.trim() || "") === "№") {
						const value = row.querySelector(".snippet__row-value")?.textContent?.trim() || "";
				...
				const creditors = [];
				li.querySelectorAll(".snippet__row").forEach((row) => {
					if ((row.querySelector(".snippet__row-key")?.textContent?.trim() || "") === "Кредиторы") {
						const valueEl = row.querySelector(".snippet__row-value");
						if (valueEl) valueEl.querySelectorAll("a.snipper__link").forEach((link) => {
							creditors.push({
								name: link.textContent?.trim() || "",
								href: link.href || ""
							});
				...
				li.querySelector(".snippet__row-key");
				let publisher = "";
				let publisherHref = "";
				rows.forEach((row) => {
					if ((row.querySelector(".snippet__row-key")?.textContent?.trim() || "") === "Публикатор") {
						const link = row.querySelector("a.snipper__link");
						if (link) {
							publisher = link.textContent?.trim() || "";
							publisherHref = link.href || "";
						}
					}
				});
				let fullText = "";
...
async function collectFoundersDetails(page, companyId, options = {}) {
		...
		for (let i = 0; i < itemCount && collected < maxTotalCases; i++) {
			...
			const parsed = await item.evaluate((li) => {
				...
				periodItems.forEach((item) => {
					if ((item.querySelector("span")?.textContent?.trim() || "") === "Период:") period = item.querySelectorAll("span")[1]?.textContent?.trim() || "";
				});
				let share = "";
				periodItems.forEach((item) => {
					if ((item.querySelector("span")?.textContent?.trim() || "") === "Доля:") share = item.querySelectorAll("span")[1]?.textContent?.trim() || "";
				});
...
async function collectReliabilityDetails(page, companyId, options = {}) {
	...
	await page.waitForSelector(".facts", { timeout: 15e3 });
	await page.waitForTimeout(1e3);
	const moreButtons = page.locator(".facts__more");
	const moreCount = await moreButtons.count();
	for (let i = 0; i < moreCount; i++) try {
	...
	for (let i = 0; i < headerCount; i++) try {
	...
	const result = await page.evaluate(() => {
		...
		const groups = [];
		document.querySelectorAll(".facts__item").forEach((groupEl) => {
			...
			if (titleEl) {
				...
				} else groupTitle = titleEl.textContent?.trim() || "";
			}
			const intro = groupEl.querySelector(".facts__intro")?.textContent?.trim() || "";
			const facts = [];
			groupEl.querySelectorAll(".accordion__item").forEach((item) => {
		...
		return {
			...
			groups,
			recommendations,
			bottom_text: document.querySelector(".bottom-block")?.textContent?.trim() || ""
		};
	});
...
async function collectSanctionsDetails(page, companyId, options = {}) {
	...
	const result = await page.evaluate(() => {
		...
		const risk_groups = [];
		document.querySelectorAll(".facts__item").forEach((factItem) => {
			const groupTitleEl = factItem.querySelector(".facts__title");
			const groupTitle = groupTitleEl ? groupTitleEl.textContent?.trim() || "" : "";
			factItem.querySelectorAll(".accordion__item").forEach((acc) => {
				acc.querySelector(".accordion__header");
		...
		const related = [];
		document.querySelectorAll(".sanctions-page__list .list-element").forEach((org) => {
			...
			org.querySelectorAll(".list-element__info-box-item").forEach((item) => {
				if ((item.querySelector("span")?.textContent?.trim() || "") === "Риски:") {
...
async function collectGzDetails(page, companyId, options = {}) {
	...
	try {
		const m = (await page.locator("div.export-data__text").first().innerText()).match(/Найдено\s*([\d\s]+)\s*закупок/);
		...
		for (let i = 0; i < itemCount && collected < maxTotalCases; i++) {
			...
			const mainData = await item.evaluate((li) => {
				...
				return {
					...
					title,
					fields,
					site_url: siteLink ? siteLink.href || "" : ""
				};
			});
			...
			if (await subSnippet.count() > 0) {
				const contractData = await subSnippet.evaluate((li) => {
					...
					const title = getText(".snippet__row-value--title");
					const fields = {};
					li.querySelectorAll("dl.snippet__block .snippet__row").forEach((row) => {
						const keyEl = row.querySelector("dt.snippet__row-key");
						const valueEl = row.querySelector("dd.snippet__row-value");
						if (!keyEl || !valueEl) return;
						const key = keyEl.textContent?.trim() || "";
...
async function applyFsspFilters(page, filters) {
	...
	if (filters.objects && Array.isArray(filters.objects) && filters.objects.length > 0) for (const obj of filters.objects) {
	...
	if (filters.due && Array.isArray(filters.due) && filters.due.length > 0) for (const due of filters.due) {
...
async function collectFsspDetails(page, companyId, options = {}) {
	...
	try {
		const m = (await page.locator("div.export-data__text").first().innerText()).match(/Найдено\s*([\d\s]+)\s*производств/);
		...
		for (let i = 0; i < itemCount && collected < maxTotalCases; i++) {
			...
			const parsed = await item.evaluate((li) => {
				...
				li.querySelectorAll(".snippet__row").forEach((row) => {
					...
					else fields[key] = valueEl.textContent?.trim() || "";
				});
				return {
					status,
					title,
...
async function applyInspectionsFilters(page, filters) {
	...
	if (filters.statuses && Array.isArray(filters.statuses) && filters.statuses.length > 0) for (const value of filters.statuses) {
		const checkbox = page.locator(`input[name="status"][value="${value}"]`);
		if (await checkbox.count() > 0 && await checkbox.isEnabled()) {
			await checkbox.check();
			await page.waitForTimeout(300);
	...
	if (filters.results && Array.isArray(filters.results) && filters.results.length > 0) for (const value of filters.results) {
...
async function collectInspectionsDetails(page, companyId, options = {}) {
	...
	try {
		const m = (await page.locator("div.export-data__text").first().innerText()).match(/Найдено\s*([\d\s]+)\s*проверок/);
		...
		for (let i = 0; i < itemCount && collected < maxTotalCases; i++) {
			...
			const parsed = await item.evaluate((li) => {
				...
				return {
					...
					title,
					fields,
					proverki_url: proverkiLink ? proverkiLink.href || "" : ""
				};
			});
...
async function collectLicensesDetails(page, companyId, options = {}) {
	...
	try {
		const m = (await page.locator("div.export-data__text").first().innerText()).match(/Найдено\s*([\d\s]+)\s*лицензии/);
		...
		for (let i = 0; i < itemCount && collected < maxTotalCases; i++) {
			...
			const parsed = await item.evaluate((li) => {
				...
				li.querySelectorAll(".snippet__row").forEach((row) => {
					...
					if (rowItems.length > 0) value = Array.from(rowItems).map((el) => el.textContent?.trim() || "").join("; ");
					const link = valueEl.querySelector("a");
					if (link) fields[key] = {
						text: link.textContent?.trim() || value,
						href: link.href || ""
...
async function collectBranchesDetails(page, companyId, options = {}) {
	...
	await page.waitForSelector(".branches-company-items", { timeout: 15e3 });
	await page.waitForTimeout(1e3);
	try {
		const m = (await page.locator(".content-frame__title").first().innerText()).match(/\((\d+)\)/);
...
async function collectHistoryDetails(page, companyId, options = {}) {
	...
	while (currentPage <= maxPages && collected < maxTotalCases) {
		...
		for (let i = 0; i < itemCount && collected < maxTotalCases; i++) {
			...
			for (let j = 0; j < togglerCount; j++) try {
				if ((await togglers.nth(j).innerText()).includes("показать все")) {
			...
			const parsed = await item.evaluate((li) => {
				...
				const groups = [];
				li.querySelectorAll(".history-box__item").forEach((groupEl) => {
				...
				if (detailsLink) detailsHref = detailsLink.href || "";
				return {
					date,
					groups,
					detailsHref
...
async function collectRequisitesDetails(page, companyId, options = {}) {
	...
	const sectionsFromUl = await page.evaluate(() => {
		...
		for (const node of childNodes) if (node.classList.contains("tile-item__title") || node.classList.contains("requisites-list__date-title")) currentTitle = node.textContent?.trim() || "";
		else if (node.classList.contains("tile-item__subtitle")) {
			if (result.length > 0) result[result.length - 1].subtitle = node.textContent?.trim() || "";
			...
			if (items.length > 0) result.push({
				title: currentTitle || "Без заголовка",
				items
			});
		}
		return result;
...
async function collectOkvedDetails(page, companyId, options = {}) {
	...
	try {
		await page.waitForSelector(".okved-list, .text-box, .content-frame__title", { timeout: 15e3 });
	} catch {
	...
	const parsed = await page.evaluate(() => {
		...
		if (contentFrameTitle.includes("Виды деятельности ОКВЭД")) {
			...
			titles.forEach((titleEl, index) => {
				...
				if (!list) return;
				list.querySelectorAll("li.okved-item").forEach((li) => {
					const num = li.querySelector(".okved-item__num")?.textContent?.trim() || "";
					const text = li.querySelector(".okved-item__text")?.textContent?.trim() || "";
					if (!num && !text) return;
					if (titleText.startsWith("Основной")) result.main_activity = `${text} (${num})`;
					else if (titleText.startsWith("Дополнительные")) result.additional_activities.push({
						code: num,
						description: text
					});
				});
		...
		const industryBlock = Array.from(document.querySelectorAll(".text-box")).find((el) => el.querySelector(".sub-title")?.textContent?.trim() === "Отрасль");
		if (industryBlock) result.industry = industryBlock.textContent?.replace("Отрасль", "").trim() || "";
		const mainBlock = Array.from(document.querySelectorAll(".text-box")).find((el) => el.querySelector(".sub-title")?.textContent?.includes("Основной вид деятельности ОКВЭД"));
		if (mainBlock) result.main_activity = mainBlock.textContent?.replace(/Основной вид деятельности ОКВЭД.*/, "").trim() || "";
		const regionBlock = Array.from(document.querySelectorAll(".text-box")).find((el) => el.querySelector(".sub-title")?.textContent?.trim() === "Регион");
		if (regionBlock) result.region = regionBlock.textContent?.replace("Регион", "").trim() || "";
		...
		if (topList) topList.querySelectorAll("li.okved-item").forEach((item) => {
			...
			if (name || position) result.top_companies.push({
				position,
				name,
				href,
				revenue
		...
		if (additionalList) {
			...
			if (list && list.classList.contains("okved-list")) list.querySelectorAll("li.okved-item").forEach((item) => {
				...
				if (code || description) result.additional_activities.push({
					code,
					description
				});
			});
...
//#endregion
//#region src/main/services/osint/scrapers/rusprofile/details/person/ceo.ts
async function collectPersonCeoDetails(page, slug) {
	...
	try {
		await page.waitForSelector(".content-frame__title, .company__list, .list-element", { timeout: 15e3 });
	} catch {
	...
	const parsed = await page.evaluate(() => {
		...
		*/
		const parseListItem = (el) => {
			...
			if (warningEl) period = warningEl.textContent?.trim() || "";
			const dashIndex = roleText.indexOf("—");
			if (dashIndex >= 0) {
				let posText = roleText.substring(dashIndex + 1).trim();
				if (period && posText.endsWith(period)) posText = posText.substring(0, posText.length - period.length).trim();
				position = posText;
			}
		...
		for (const child of children) {
			...
			if (el.classList.contains("list-element__row") && currentSection) el.querySelectorAll(".list-element").forEach((item) => {
				const parsed = parseListItem(item);
				if (parsed.name || parsed.inn) result[currentSection].push(parsed);
			});
		}
		return result;
	});
...
//#endregion
//#region src/main/services/osint/scrapers/rusprofile/details/person/founder.ts
async function collectPersonFounderDetails(page, slug) {
	...
	const parsed = await page.evaluate(() => {
		...
		*/
		const parseListItem = (el) => {
			...
			const statusEl = el.querySelector(".warning-text");
			if (statusEl) status = statusEl.textContent?.trim() || "";
			const activity = el.querySelectorAll(".list-element__text")[0]?.textContent?.trim() || "";
			let period = "";
			let share = "";
...
async function collectPersonReliabilityDetails(page, slug) {
...
async function collectPersonHistoryDetails(page, slug) {
...
async function scrapeRusprofile(inn, options) {
	...
	try {
		...
		const shouldCollect = (section) => !options?.onlySections || options.onlySections.includes(section);
		const loginTrigger = page.locator("#menu-personal-trigger");
		await loginTrigger.waitFor({
			state: "visible",
			timeout: 15e3
		...
		if (entityType !== "person") {
			...
			if (options?.trademarksDetails && shouldCollect("trademarks_details")) {
				...
				result.trademarks_details = await timed("trademarks_details", () => collectTrademarksDetails(page, companyId, {
			...
			if (options?.leasingDetails && shouldCollect("leasing_details")) {
				...
				result.leasing_details = await timed("leasing_details", () => collectLeasingDetails(page, companyId, {
			...
			if (options?.pledgesDetails && shouldCollect("pledges_details")) {
				...
				result.pledges_details = await timed("pledges_details", () => collectPledgesDetails(page, companyId, {
			...
			if (options?.factsDetails && shouldCollect("facts_details")) {
				...
				result.facts_details = await timed("facts_details", () => collectFactsDetails(page, companyId, {
			...
			if (options?.bankruptcyDetails && shouldCollect("bankruptcy_details")) {
				...
				result.bankruptcy_details = await timed("bankruptcy_details", () => collectBankruptcyDetails(page, companyId, {
			...
			if (options?.foundersDetails && shouldCollect("founders_details")) {
				...
				result.founders_details = await timed("founders_details", () => collectFoundersDetails(page, companyId, {
			...
			if (options?.reliabilityDetails && shouldCollect("reliability_details")) {
				...
				result.reliability_details = await timed("reliability_details", () => collectReliabilityDetails(page, companyId));
			}
			if (options?.sanctionsDetails && shouldCollect("sanctions_details")) {
				console.log("Сбор детальных санкций...");
				result.sanctions_details = await timed("sanctions_details", () => collectSanctionsDetails(page, companyId));
			}
			if (options?.gzDetails && shouldCollect("gz_details")) {
				console.log("Сбор детальных госзакупок...");
				result.gz_details = await timed("gz_details", () => collectGzDetails(page, companyId, {
			...
			if (options?.fsspDetails && shouldCollect("fssp_details")) {
				...
				result.fssp_details = await timed("fssp_details", () => collectFsspDetails(page, companyId, {
			...
			if (options?.inspectionsDetails && shouldCollect("inspections_details")) {
				...
				result.inspections_details = await timed("inspections_details", () => collectInspectionsDetails(page, companyId, {
			...
			if (options?.licensesDetails && shouldCollect("licenses_details")) {
				...
				result.licenses_details = await timed("licenses_details", () => collectLicensesDetails(page, companyId, {
			...
			if (options?.branchesDetails && shouldCollect("branches_details")) {
				...
				result.branches_details = await timed("branches_details", () => collectBranchesDetails(page, companyId));
			}
			if (options?.historyDetails && shouldCollect("history_details")) {
				console.log("Сбор детальной истории...");
				result.history_details = await timed("history_details", () => collectHistoryDetails(page, companyId, {
			...
			if (options?.requisitesDetails && shouldCollect("requisites_details")) {
				...
				result.requisites_details = await timed("requisites_details", () => collectRequisitesDetails(page, companyId));
			}
			if (options?.okvedDetails && shouldCollect("okved_details")) {
				console.log("Сбор детальных видов деятельности...");
				result.okved_details = await timed("okved_details", () => collectOkvedDetails(page, companyId));
			}
		...
		if (entityType === "person") {
			...
			if (options?.personCeoDetails && shouldCollect("person_ceo_details")) {
				...
				result.person_ceo_details = await timed("person_ceo_details", () => collectPersonCeoDetails(page, slug));
			}
			...
			if (options?.personConnectionsDetails && shouldCollect("person_connections_details")) {
				...
				result.person_connections_details = await timed("person_connections_details", () => collectPersonConnectionsDetails(page, slug));
			}
			if (options?.personReliabilityDetails && shouldCollect("person_reliability_details")) {
				console.log("Сбор факторов риска (ФЛ)...");
				result.person_reliability_details = await timed("person_reliability_details", () => collectPersonReliabilityDetails(page, slug));
			}
			if (options?.personHistoryDetails && shouldCollect("person_history_details")) {
				console.log("Сбор истории (ФЛ)...");
				result.person_history_details = await timed("person_history_details", () => collectPersonHistoryDetails(page, slug));
			}
		}
...
//#endregion
//#region src/shared/sensitive/wordlists/bip39-en.ts
var BIP39_EN = [
	"abandon",
	"ability",
	"able",
...
//#endregion
//#region src/shared/sensitive/wordlists/bip39-ru.ts
var BIP39_RU = [
	"абажур",
	"абзац",
	"аванс",
...
var cleanedEnCache = null;
var cleanedRuCache = null;
function getCleanedList(lang) {
	if (lang === "en") {
...
function getWordlist(lang) {
	...
	if (list.length < MIN_WORDLIST_SIZE) throw new Error(`Словник "${lang}" слишком короткий после очистки (${list.length}). Минимум ${MIN_WORDLIST_SIZE} слов.`);
	return list;
}
...
var OrderManager = class {
	...
	async processSignal(signal) {
		...
		let balanceRes = null;
		try {
			balanceRes = await sandboxGrpc.getSandboxPortfolio({ accountId: this.config.accountId }, this.config.token);
		} catch (e) {
		...
		if (this.config.useDynamicSizing) {
			...
			if (riskPerLot > 0 && riskAmount > 0) {
				...
				const entryPrice = signal.price || signal.targetPrice || 0;
				const portfolioTotal = Number(balanceRes?.totalAmountPortfolio?.units || 0) + (balanceRes?.totalAmountPortfolio?.nano || 0) / 1e9;
				const securitiesValue = portfolioTotal - (Number(balanceRes?.totalAmountCurrencies?.units || 0) + (balanceRes?.totalAmountCurrencies?.nano || 0) / 1e9);
				const newPositionCost = quantity * entryPrice;
				const currentInstrumentPosition = (balanceRes?.positions || []).find((p) => p.instrumentUid === signal.instrumentUid);
				const currentInstrumentValue = (currentInstrumentPosition?.quantity ? Math.abs(Number(currentInstrumentPosition.quantity.units) + (currentInstrumentPosition.quantity.nano || 0) / 1e9) : 0) * entryPrice;
...
var DeepSeekService = class DeepSeekService {
	...
	*/
	async gotoWithTimeout(url, timeoutMs = 15e3) {
		if (!this.page) return;
		try {
			...
			await this.page.evaluate(() => {
				...
				document.querySelectorAll(".loading, .loader, .spinner, [class*=\"loading\"], [class*=\"spinner\"]").forEach((el) => {
					el.style.display = "none";
				});
				if (typeof window.jQuery !== "undefined") window.jQuery(window).trigger("load");
			});
			console.log(`[DeepSeek] Переход на ${url} завершён, троббер остановлен`);
		} catch (error) {
			console.warn(`[DeepSeek] Переход на ${url} не завершился полностью: ${error.message}`);
		}
	}
	/**
	...
	*/
	async stopLoadingSpinner() {
		...
		try {
			...
			await page.locator("div.ds-dropdown-menu").waitFor({
				state: "visible",
				timeout: 5e3
			});
	...
	*/
	async login() {
		...
		await this.gotoWithTimeout(LOGIN_URL, 2e4);
		...
		try {
			...
			await this.page.waitForSelector(textareaSelector, { timeout: 2e4 });
	...
	*/
	async ensureReadyForChat() {
		...
		if (await this.page.locator(textareaSelector).count() > 0) return;
		console.log("Интерфейс чата не обнаружен, пробуем выбрать режим...");
		const fastMode = this.page.locator("[data-model-type=\"default\"][role=\"radio\"]");
		await fastMode.waitFor({
			state: "visible",
			timeout: 1e4
		});
		await fastMode.click();
		await this.page.waitForSelector(textareaSelector, { timeout: 15e3 });
	...
	*/
	async startSelectionMode() {
		if (!this.page) throw new Error("Браузер не запущен");
		const button = this.page.locator("div._57370c5._5dedc1e").first();
	...
	*/
	async selectMessages(indices) {
		...
		await this.page.locator("div.ds-virtual-list").waitFor({
			state: "visible",
			timeout: 5e3
		});
		...
		for (const index of sorted) {
			...
			await this.page.locator(`div[data-virtual-list-item-key]`).nth(index).locator(".d30139ff .ds-checkbox").click({ force: true });
	...
	*/
	async scrollToMessageIndex(index) {
		const page = this.page;
		const listSelector = "div.ds-virtual-list";
		if (await page.evaluate(({ listSelector, index }) => {
			const list = document.querySelector(listSelector);
			if (!list) return false;
			return index < list.querySelectorAll("div[data-virtual-list-item-key]").length;
		}, {
			listSelector,
			index
		})) {
			await page.evaluate(({ listSelector, index }) => {
				const items = document.querySelector(listSelector)?.querySelectorAll("div[data-virtual-list-item-key]");
				if (items && items[index]) items[index].scrollIntoView({ block: "center" });
			}, {
				listSelector,
				index
			});
	...
	*/
	async regenerateMessage(conversationId, messageIndex) {
		if (!this.page) throw new Error("Браузер не запущен");
		if (!this.page.url().includes(conversationId)) await this.openConversation(conversationId);
		await this.page.waitForSelector("div.ds-markdown.ds-assistant-message-main-content", { timeout: 1e4 });
		const assistantMessages = this.page.locator("div.ds-markdown.ds-assistant-message-main-content");
		const count = await assistantMessages.count();
		if (messageIndex >= count) throw new Error(`Индекс сообщения вне диапазона: ${messageIndex}, всего ${count}`);
		const repeatButton = assistantMessages.nth(messageIndex).locator("xpath=ancestor::div[contains(@class, \"_4f9bf79\")]").first().locator("div.db183363").nth(1);
		await repeatButton.waitFor({
			state: "visible",
			timeout: 5e3
		});
	...
	*/
	async sendFeedback(conversationId, messageIndex, type) {
		if (!this.page) throw new Error("Браузер не запущен");
		if (!this.page.url().includes(conversationId)) await this.openConversation(conversationId);
		await this.page.waitForSelector("div.ds-markdown.ds-assistant-message-main-content", { timeout: 1e4 });
		const assistantMessages = this.page.locator("div.ds-markdown.ds-assistant-message-main-content");
		if (messageIndex >= await assistantMessages.count()) throw new Error("Индекс сообщения вне диапазона");
		const messageContainer = assistantMessages.nth(messageIndex).locator("xpath=ancestor::div[contains(@class, \"_4f9bf79\")]").first();
		const buttonIndex = type === "like" ? 2 : 3;
...
function registerGatewayHandlers() {
	...
	electron.ipcMain.handle("gateway:send-message", async (_event, message) => {
		try {
			return {
				success: true,
	...
	electron.ipcMain.handle("gateway:start-selection-mode", async () => {
		try {
	...
	electron.ipcMain.handle("gateway:select-messages", async (_event, indices) => {
		try {
	...
	electron.ipcMain.handle("gateway:cancel-selection-mode", async () => {
		try {
	...
	electron.ipcMain.handle("gateway:create-public-link", async () => {
		try {
			return {
				success: true,
				data: await service.createPublicLink()
			};
		} catch (error) {
			return {
				success: false,
	...
	electron.ipcMain.handle("gateway:regenerate-message", async (_event, conversationId, messageIndex) => {
		try {
	...
	electron.ipcMain.handle("gateway:send-feedback", async (_event, conversationId, messageIndex, type) => {
		try {
			await service.sendFeedback(conversationId, messageIndex, type);
			return { success: true };
		} catch (error) {
			return {
				success: false,
